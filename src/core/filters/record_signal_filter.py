...
def check_time_condition(previous_snapshot: SignalSnapshot, record_signal_filter: Record_signal_filter) -> Tuple[bool, str]:
    ...
    time_gap = round((now_time - previous_snapshot.recorded_at).total_seconds(), 2)
    time_condition = time_gap >= time_window_seconds
    if not time_condition:
        details += f"时间间隔 {time_gap} 不大于等于要求的 {time_window_seconds} 秒 \n"
    else:
        details += f"时间间隔 {time_gap} 符合要求的 {time_window_seconds} 秒 \n"
...
def check_ev_change_condition(
    ...
    now_roi_relative_change = round(abs(now_snapshot.roi_pct - previous_snapshot.roi_pct), 2)
    now_net_ev_absolute_change = round(abs(now_snapshot.net_ev - previous_snapshot.net_ev), 2)
    ev_change_condition = (
...
def check_market_change_condition(
    ...
    now_pm_price_pct_change = round((now_pm_price_change / pm_base) * 100, 2)
    now_deribit_price_pct_change = round((now_deribit_price_change / deribit_base) * 100, 2)
    ...
    if not market_change_condition:
        ...
        details += (
            f"PM 价格变化 {now_pm_price_pct_change} 符合要求的 {pm_price_pct_change}"
            f"Deribit 期权价格变化 {now_deribit_price_pct_change} 符合要求的 {deribit_price_pct_change}"
        )
